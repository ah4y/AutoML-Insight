...
def plot_calibration_curve(calibration_data: Dict, class_idx: int = 0) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=[0, 1], y=[0, 1], mode="lines", name="Perfect Calibration", line=dict(color="gray", width=2, dash="dash")
        )
    )
...
def plot_bias_variance_comparison(models_bv_data: Dict[str, Dict]) -> go.Figure:
    ...
    fig.add_trace(go.Bar(name="Bias²", x=model_names, y=bias_values, marker_color=TRAIN_COLOR))

    fig.add_trace(go.Bar(name="Variance", x=model_names, y=variance_values, marker_color=TEST_COLOR))
...
def plot_cv_stability_boxplot(cv_scores_dict: Dict[str, List[float]]) -> go.Figure:
    ...
    for model_name, scores in cv_scores_dict.items():
        if scores and len(scores) > 0:
            fig.add_trace(go.Box(y=scores, name=model_name, boxmean="sd"))  # Show mean and standard deviation
