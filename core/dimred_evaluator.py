...
from core.dimred import DimRedConfig, make_dimred, should_enable_dimred
...
class DimRedEvaluator:
    ...
    def evaluate_classification_with_dimred(
        ...
        for model_name, model in models.items():
            ...
            if not model_benefits_from_dimred and self.dimred_config.enable == 'auto':
                ...
                results[model_name] = self._evaluate_single_model(
                    model, X_raw, y, model_name, use_dimred=False
                )
                continue
            
            # Evaluate without dimred (baseline)
            baseline_results = self._evaluate_single_model(
                model, X_raw, y, f"{model_name}_baseline", use_dimred=False
            )
            
            # Evaluate with dimred (if enabled)
            dimred_results = None
            if self.dimred_config.enable != 'off':
                dimred_results = self._evaluate_single_model(
                    model, X_raw, y, f"{model_name}_dimred", use_dimred=True
                )
            
            # Compare and select best variant
            selected_results = self._compare_and_select_variant(
                model_name, baseline_results, dimred_results
            )
        ...
        if self.dimred_config.enable != 'off':
            self.logger.info(f"Fitting PCA transformer on full dataset for visualization (config: enable={self.dimred_config.enable})")
            try:
                ...
                if pca_transformer is not None:
                    ...
                    self.logger.info(f"PCA transformer fitted with {X_transformed.shape[1]} components for visualization")
                else:
    ...
    def _compare_and_select_variant(
        ...
        dimred_results: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"{model_name}: Selected {selected['selected_variant']} "
