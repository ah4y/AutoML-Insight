...
class EnhancedAIInsightEngine:
    ...
    def generate_comprehensive_insights(
        self,
        stats: EnhancedDatasetStatistics,
        ...
        try:
            ...
            if "rate_limit" in error_msg.lower() or "429" in error_msg:
                insights['_notice'] = "⚠️ AI rate limit reached. Showing enhanced rule-based analysis. Upgrade your API plan or try again later for AI insights."
            else:
  ...
  "research_directions": ["Advanced techniques or research areas relevant to this dataset"]
    ...
    def _make_llm_api_call(self, prompt: str) -> str:
        ...
        if self.provider == 'groq':
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert data scientist and ML engineer with deep expertise in dataset analysis, statistical insights, and machine learning. Always provide comprehensive, actionable insights in valid JSON format."},
    ...
    def _generate_enhanced_fallback_insights(self, stats: EnhancedDatasetStatistics) -> Dict[str, Any]:
        ...
        if stats.missing_rate > 0.15:
            challenges.append(f"Very high missing data rate ({stats.missing_rate:.1%}) requires robust imputation strategy")
        ...
        if stats.class_balance:
            ...
            if imbalance_ratio > 20:
                challenges.append(f"Severe class imbalance (ratio {imbalance_ratio:.1f}:1) requires specialized techniques")
            elif imbalance_ratio > 10:
                challenges.append(f"Significant class imbalance (ratio {imbalance_ratio:.1f}:1) needs balancing strategies")
        ...
        if high_cardinality:
            challenges.append(f"High-cardinality categorical features need encoding optimization")
        ...
        if stats.variance_summary.get('low_variance', 0) > 0:
            statistical.append(f"{stats.variance_summary['low_variance']} features have low variance and may be uninformative")
        
        if stats.feature_correlations:
            max_corr = stats.feature_correlations[0][2]
            statistical.append(f"Maximum feature correlation is {max_corr:.3f}, indicating {'high' if max_corr > 0.8 else 'moderate' if max_corr > 0.5 else 'low'} multicollinearity")
        
        if stats.correlation_with_target:
            max_target_corr = stats.correlation_with_target[0][1]
            statistical.append(f"Strongest target correlation is {max_target_corr:.3f}, suggesting {'strong' if max_target_corr > 0.7 else 'moderate' if max_target_corr > 0.3 else 'weak'} linear relationships")
