...
class StackingEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_models: List[Any], meta_model: Any = None, use_probas: bool = True):
...
class AdaptiveEnsemble:
    ...
    def create_ensemble(
        ...
        leaderboard = sorted(evaluation_results.items(), key=lambda x: x[1].get("accuracy_mean", 0), reverse=True)[
            :top_k
        ]
        ...
        if len(top_models) >= 3:
            ...
            self.ensemble = StackingEnsemble(base_models=top_models, use_probas=True)
            self.ensemble_type = "stacking"
        else:
            # Use weighted ensemble for fewer models
            self.ensemble = WeightedEnsemble(models=top_models, weights=weights)
    ...
    def get_ensemble_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "n_models": len(self.ensemble.models if hasattr(self.ensemble, "models") else self.ensemble.base_models),
