...
class AdvancedHyperparameterOptimizer:
    ...
    def __init__(
        self, task_type="classification", optimization_time_minutes=10, n_trials=100, random_state=42, verbose=True
    ):
        """
    ...
    def _initialize_search_spaces(self):
        ...
        return {
            ...
            # Neural networks
            "MLPClassifier": {
                "hidden_layer_sizes": ("categorical", [(50,), (100,), (200,), (100, 50), (200, 100), (300, 150, 75)]),
    ...
    def optimize_model(self, model, X, y=None, cv_folds=5, scoring_strategy="balanced"):
        ...
        study = optuna.create_study(
            direction="maximize", sampler=self.sampler, pruner=self.pruner, study_name=study_name
        )
        ...
        try:
            study.optimize(objective, n_trials=self.n_trials, timeout=timeout, show_progress_bar=self.verbose)
        except Exception as e:
            logger.warning(f"Optimization interrupted for {model_name}: {e}")
            if self.verbose:
        ...
        if len(completed_trials) == 0:
            ...
            return {
                ...
                "convergence_info": {"converged": False, "convergence_iteration": None, "final_score": baseline_score},
            }
        ...
        results = {
            ...
            "n_trials": len(study.trials),
            "study": study,
            "convergence_info": {"best_trial": study.best_trial.number, "trials_to_best": study.best_trial.number + 1},
        }
    ...
    def _objective_function(self, trial, base_model, X, y, cv_folds, scoring_strategy, model_name):
        ...
        try:
            ...
            logger.debug(f"Failed to evaluate model {model_name}: {e}")
            if self.verbose:
    ...
    def _evaluate_clustering_model(self, model, X):
        ...
        try:
            ...
            logger.warning(f"Clustering trial evaluation failed, scoring as -1000: {e}")
            return -1000
    ...
    def optimize_ensemble(self, models, X, y, ensemble_type="voting", cv_folds=5):
        ...
        if ensemble_type == "voting":
            ...
            if self.task_type == "classification":
                ...
                ensemble = StackingClassifier(estimators=optimized_models, final_estimator=meta_learner, cv=cv_folds)
            else:
                from sklearn.linear_model import LinearRegression

                meta_learner = LinearRegression()
                ensemble = StackingRegressor(estimators=optimized_models, final_estimator=meta_learner, cv=cv_folds)
        ...
        individual_scores = [
            self.optimization_results[name.split("_")[0]]["best_score"]
...
class AutoMLPipeline:
    ...
    def __init__(self, task_type="classification", optimization_time_minutes=15, random_state=42):
        ...
        self.optimizer = AdvancedHyperparameterOptimizer(
            task_type=task_type, optimization_time_minutes=optimization_time_minutes, random_state=random_state
        )
    ...
    def run_advanced_automl(self, X, y=None, model_candidates=None, include_ensemble=True):
        ...
        for i, (name, model) in enumerate(model_candidates):
            ...
            try:
                ...
                logger.error(f"{name} optimization failed: {e}", exc_info=True)
        ...
        successful_models = [
            name for name, result in individual_results.items() if not result.get("optimization_failed", False)
        ]
        ...
        self.pipeline_results = {
            ...
            "dataset_info": {"n_samples": X.shape[0], "n_features": X.shape[1], "task_type": self.task_type},
            "pipeline_stats": {
    ...
    def get_best_model(self):
        ...
        return {"name": best_name, "model": best_model, "score": best_score}
