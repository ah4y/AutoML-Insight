...
from .dimred import DimRedConfig, select_dimred_method, should_enable_dimred
...
class MetaModelSelector:
    ...
    def fit(self, meta_features_list: List[np.ndarray], best_models: List[str]):
        """
        ...
        # Train meta-model
        self.meta_model = GradientBoostingClassifier(n_estimators=100, random_state=self.random_state)
    ...
    def get_recommendation_with_rationale(
        self, meta_features: Dict[str, Any], evaluation_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        # Performance-based
        rationale.append(f"{best_model} achieved the highest accuracy: {best_score:.4f}")
        ...
        if len(leaderboard) > 1:
            ...
            if diff > 0.01:
                rationale.append(f"Significantly outperforms {second_best} by {diff:.4f}")

        # Dataset characteristics
        meta_features.get("n_samples", 0)
        meta_features.get("n_features", 0)

        if "RandomForest" in best_model or "XGBoost" in best_model:
            rationale.append("Tree-based ensemble well-suited for complex feature interactions")

        if "MLP" in best_model:
            rationale.append("Neural network effective for large datasets with non-linear patterns")

        if "SVM" in best_model:
            rationale.append("Support Vector Machine handles high-dimensional spaces effectively")
        ...
        if ci_width < 0.05:
            rationale.append(f"Model performance is stable (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}])")
        ...
        return {
            ...
            "rationale": rationale,
            "alternatives": [
                {"model": model, "score": results.get("accuracy_mean", 0)} for model, results in leaderboard[1:4]
            ],
        }

    def recommend_dimred_config(
        self, meta_features: Dict[str, Any], dimred_evaluation_results: Optional[Dict[str, Any]] = None
    ) -> DimRedConfig:
        """
    ...
    def get_dimred_recommendation_rationale(
        ...
        if dimred_config.enable == "off":
            rationale.append(
                f"Dataset has {n_features} features for {n_samples} samples (ratio: {dimensionality_ratio:.3f})"
            )
            ...
            if n_features > 1000:
                rationale.append(
                    f"High-dimensional dataset ({n_features} features) - dimensionality reduction recommended"
                )
                impact_score = 0.8
            elif dimensionality_ratio > 0.5:
                rationale.append(
                    f"Feature-rich dataset (ratio: {dimensionality_ratio:.3f}) - may benefit from dimensionality reduction"
                )
                impact_score = 0.6
            ...
            if dimred_config.variance_target < 0.95:
                rationale.append(
                    f"Aggressive dimensionality reduction (target: {dimred_config.variance_target:.0%}) for high-dimensional data"
                )
            else:
                rationale.append(
                    f"Conservative reduction (target: {dimred_config.variance_target:.0%}) to preserve information"
                )
