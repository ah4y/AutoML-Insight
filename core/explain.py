...
class ModelExplainer:
    ...
    def explain_model(
        self, model: Any, X: np.ndarray, feature_names: Optional[list] = None, sample_size: int = 50
    ) -> dict:
        """
        ...
        # Coefficient importance (for linear models)
            ...
            if coef.ndim > 1:
                ...
                coef = np.abs(coef)
            explanations["coef_importance"] = {name: float(val) for name, val in zip(self.feature_names, coef)}
    ...
    def plot_shap_summary(self, save_path: Optional[str] = None, show: bool = True):
        """
        ...
        if isinstance(self.shap_values, list):
            ...
            shap.summary_plot(self.shap_values[0], features=None, feature_names=self.feature_names, show=False)
        else:
            shap.summary_plot(self.shap_values, features=None, feature_names=self.feature_names, show=False)
    ...
    def get_top_features(self, explanations: dict, top_n: int = 10) -> list:
        """
    ...
    def compute_permutation_importance(self, model: Any, X: np.ndarray, y: np.ndarray, random_state: int = 42) -> dict:
        """
        Compute permutation importance.
