...
class Visualizer:
    ...
    def plot_leaderboard(
        self, leaderboard: List[Dict[str, Any]], metric_name: str = "Accuracy", title: str = "Model Leaderboard"
    ) -> go.Figure:
        """
    ...
    def plot_roc_curves(
        self, models_data: Dict[str, Dict[str, Any]], X: np.ndarray, y: np.ndarray, n_classes: int
    ) -> go.Figure:
        """
        ...
        for model_name, data in models_data.items():
            ...
            try:
                ...
                if n_classes == 2:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=fpr, y=tpr, mode="lines", name=f"{model_name} (AUC={roc_auc:.3f})", line=dict(width=2)
                        )
                    )
                else:
                    # Macro-average ROC
                ...
                logger.debug(f"ROC curve trace skipped for '{model_name}': {e}")
                continue
    ...
    def plot_confusion_matrix(
        ...
        fig.update_layout(
            title="Confusion Matrix", xaxis_title="Predicted", yaxis_title="True", height=500, template="plotly_white"
        )

        return fig

    def plot_calibration_curve(self, models_data: Dict[str, Dict[str, Any]], X: np.ndarray, y: np.ndarray) -> go.Figure:
        """
        ...
        fig.add_trace(
            go.Scatter(x=[0, 1], y=[0, 1], mode="lines", name="Perfect", line=dict(dash="dash", color="gray"))
        )
    ...
    def plot_feature_importance(
        self, importance_dict: Dict[str, float], top_n: int = 15, title: str = "Feature Importance"
    ) -> go.Figure:
        """
        ...
        sorted_features = sorted(importance_dict_clean.items(), key=lambda x: x[1], reverse=True)[:top_n]

        features, importances = zip(*sorted_features)

        fig = go.Figure()

        fig.add_trace(go.Bar(x=list(importances), y=list(features), orientation="h", marker_color="coral"))

        fig.update_layout(
            title=title, xaxis_title="Importance", yaxis_title="Feature", height=500, template="plotly_white"
        )

        return fig

    def plot_elbow_curve(self, k_range: range, inertias: List[float]) -> go.Figure:
        """
    ...
    def plot_silhouette_scores(self, k_range: range, silhouette_scores: List[float]) -> go.Figure:
        """
    ...
    def plot_umap_projection(self, X: np.ndarray, labels: np.ndarray, title: str = "UMAP Projection") -> go.Figure:
        """
        ...
        Returns:
            Plotly figure
        """
        df = pd.DataFrame({"UMAP1": X[:, 0], "UMAP2": X[:, 1], "Cluster": labels.astype(str)})

        fig = px.scatter(df, x="UMAP1", y="UMAP2", color="Cluster", title=title, template="plotly_white", height=500)

        return fig

    def plot_correlation_heatmap(self, data: pd.DataFrame, title: str = "Feature Correlation Heatmap") -> go.Figure:
        """
    ...
    def plot_pca_scree(
        self, explained_variance_ratio: np.ndarray, n_components_selected: int = None, title: str = "PCA Scree Plot"
    ) -> go.Figure:
        """
        ...
        fig.add_trace(
            go.Bar(x=components, y=explained_variance_ratio, name="Individual", marker_color="steelblue", opacity=0.7),
            row=1,
            col=1,
        )
        ...
        if n_components_selected is not None and n_components_selected > 0:
            ...
            n_comp_int = (
                int(n_components_selected) if not isinstance(n_components_selected, int) else n_components_selected
            )
    ...
    def plot_pca_2d_scatter(
        ...
        plot_data = {"PC1": X_pca[:, 0], "PC2": X_pca[:, 1] if X_pca.shape[1] > 1 else np.zeros(len(X_pca))}
