...
class ClusteringEvaluator:
    ...
    def evaluate_model(self, model: Any, X: np.ndarray, model_name: str, labels: np.ndarray = None) -> Dict[str, Any]:
        """
    ...
    def _compute_stability(self, model: Any, X: np.ndarray, n_iterations: int = 10) -> float:
        """
        ...
        splitter = ShuffleSplit(n_splits=n_iterations, test_size=0.3, random_state=self.random_state)
        ...
        for train_idx, test_idx in splitter.split(X):
            try:
                ...
                if np.sum(valid_mask) > 0:
                    ari = adjusted_rand_score(base_labels_train[valid_mask], new_labels[valid_mask])
    ...
    def get_leaderboard(self, metric: str = "silhouette") -> list:
        ...
        for model_name, results in self.results.items():
            if metric in results:
                ...
                leaderboard.append({"model": model_name, "score": score, "n_clusters": results["n_clusters"]})
