...
class StatisticalModelComparator:
    ...
    def compute_friedman_test(self, results_dict):
        ...
        try:
            ...
            model_rankings = {model_names[i]: float(mean_ranks[i]) for i in range(len(model_names))}
    ...
    def compute_cv_stability(self, cv_scores_list):
        ...
        try:
            ...
            logger.info(
                f"CV stability: mean={mean_score:.4f}, std={std_score:.4f}, " f"stability_index={stability_index:.4f}"
            )
