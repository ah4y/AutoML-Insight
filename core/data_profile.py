...
class DataProfiler:
        ...
        if len(numeric_cols) > 0:
            ...
            try:
                if X_numeric.shape[0] > X_numeric.shape[1]:
                    ...
                    profile["pca_95_components"] = np.sum(np.cumsum(pca.explained_variance_ratio_) < 0.95) + 1
    ...
    def get_profile_vector(self) -> np.ndarray:
        ...
        if "class_entropy" in self.profile:
            feature_names.extend(["n_classes", "class_imbalance", "class_entropy", "linear_separability"])
