...
class AIInsightEngine:
    ...
    def generate_insights(self, stats: DatasetStatistics, context: str = "initial_analysis") -> Dict[str, str]:
        """
        ...
        try:
            ...
            if "rate_limit" in error_msg.lower() or "429" in error_msg:
                insights["_notice"] = (
                    "⚠️ AI rate limit reached. Showing rule-based recommendations. Upgrade your API plan or try again later for AI insights."
                )
            else:
        ...
        if self.provider == "groq":
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert data scientist providing concise, actionable insights. Always respond in valid JSON format.",
                    },
    ...
    def _fallback_insights(self, stats: DatasetStatistics) -> Dict[str, str]:
        ...
        if stats.class_balance:
            ...
            if imbalance_ratio > 10:
                challenges.append(
                    f"Severe class imbalance (ratio {imbalance_ratio:.1f}:1) - use SMOTE or class weights"
                )
            elif imbalance_ratio > 5:
                challenges.append(
                    f"Moderate class imbalance (ratio {imbalance_ratio:.1f}:1) - monitor minority class performance"
                )
        ...
        if stats.data_quality_score < 60:
            insights["data_quality"] = (
                f"⚠️ Data quality score is low ({stats.data_quality_score:.0f}/100). Address missing data and class imbalance before training."
            )
        elif stats.data_quality_score < 80:
            insights["data_quality"] = (
                f"Data quality is moderate ({stats.data_quality_score:.0f}/100). Some preprocessing recommended."
            )
        else:
