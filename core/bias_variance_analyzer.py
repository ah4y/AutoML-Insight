    ...
    def compute_bias_variance_decomposition(self, model, X_train, y_train, X_test, y_test):
        ...
        try:
            ...
            main_predictions = np.apply_along_axis(
                lambda x: np.bincount(x.astype(int), minlength=n_classes).argmax(), axis=0, arr=predictions
            )
            ...
            variance = np.mean([np.mean(predictions[i, :] != main_predictions) for i in range(self.n_bootstrap)])
            ...
            logger.info(
                f"Bias-variance decomposition complete: bias²={bias_squared:.4f}, "
