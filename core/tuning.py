...
class OptunaHyperparameterTuner:
    ...
    def __init__(
        self, n_trials: int = 20, cv: int = 3, scoring: str = "accuracy", random_state: int = 42, verbose: bool = False
    ):
    ...
    def tune(self, model_name: str, model: Any, X: np.ndarray, y: np.ndarray) -> Any:
        ...
        def objective(trial):
            ...
            # Cross-validation
            try:
                scores = cross_val_score(tuned_model, X, y, cv=self.cv, scoring=self.scoring, n_jobs=-1)
                return scores.mean()
            except Exception as e:
                logger.warning(f"Trial evaluation failed, scoring as 0.0: {e}")
                return 0.0
        ...
        if model_name == "RandomForest":
            ...
            return {
                "hidden_layers": trial.suggest_categorical(
                    "hidden_layers", [(64,), (128,), (64, 32), (128, 64), (256, 128)]
                ),
