...
class ClassificationEvaluator:
    ...
    def __init__(self, n_folds: int = 5, n_repeats: int = 3, random_state: int = 42):
        self.n_folds = n_folds
        self.n_repeats = n_repeats
        self.random_state = random_state
        self.results = {}

    def evaluate_model(self, model: Any, X: np.ndarray, y: np.ndarray, model_name: str) -> Dict[str, Any]:
        """
    ...
    def compare_models(self, model1_name: str, model2_name: str) -> Dict[str, float]:
        """
        ...
        try:
            mcnemar_p = mcnemar_test(results1["true_labels"], results1["predictions"], results2["predictions"])
        ...
        if "accuracy_scores" in results1 and "accuracy_scores" in results2:
            try:
                wilcoxon_p = wilcoxon_test(np.array(results1["accuracy_scores"]), np.array(results2["accuracy_scores"]))
                ...
                leaderboard.append(
                    {
                        ...
                        "overfitting_gap": abs(
                            results.get("train_accuracy", score) - results.get("test_accuracy", score)
                        ),
                    }
                )
    ...
    def evaluate_with_holdout(
        ...
        if cv_sample_size < n_train:
            ...
            X_cv, _, y_cv, _ = train_test_split(
                X_train, y_train, train_size=cv_sample_size, stratify=y_train, random_state=42
            )
            logger.info(
                f"Large dataset ({n_train:,} samples). Using {cv_sample_size:,} samples for fast CV with {n_folds} folds."
            )
        else:
            X_cv, y_cv = X_train, y_train
        ...
        if "SVM" in model_name and n_train > 20000:
            ...
            X_train_subset, _, y_train_subset, _ = train_test_split(
                X_train, y_train, train_size=max_svm_samples, stratify=y_train, random_state=42
            )
        ...
        extended_metrics = advanced_calc.compute_extended_metrics(
            y_test, y_test_pred, y_proba=model.predict_proba(X_test) if hasattr(model, "predict_proba") else None
        )
        ...
        if hasattr(model, "predict_proba"):
            ...
            confidence_analysis = advanced_calc.compute_prediction_confidence_analysis(y_test, y_test_pred, y_proba)
        else:
            calibration = None
            confidence_analysis = None

        # Compute detailed confusion matrix analysis
        confusion_analysis = advanced_calc.analyze_confusion_matrix_detailed(y_test, y_test_pred, class_names=None)
        ...
        bv_future = st.session_state.bg_manager.schedule_analysis(
            bv_analyzer.compute_bias_variance_decomposition, model, X_train, y_train, X_test, y_test
        )

        # Learning curves (background)
        lc_future = st.session_state.bg_manager.schedule_analysis(
            bv_analyzer.compute_learning_curves, model, X_train, y_train
        )

        # Detect overfitting (populates detector state consumed by get_user_guidance() below)
        ...
        detector.detect_overfitting(
            ...
            dataset_info={
                ...
                "class_balance": {int(i): float(np.sum(y_test == i) / len(y_test)) for i in np.unique(y_test)},
            },
        )
