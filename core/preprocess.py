...
class DataPreprocessor:
    ...
    def fit_transform(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        if self.categorical_features:
            ...
            if high_cardinality_features:
                self.logger.warning(
                    f"Found {len(high_cardinality_features)} high-cardinality categorical features that would create {sum(n for _, n in high_cardinality_features):,} one-hot encoded columns"
                )
                self.logger.warning(
                    f"Removing these features to prevent memory issues: {[col for col, _ in high_cardinality_features[:5]]}"
                )

                # Remove high cardinality categorical features
                X = X.drop(columns=[col for col, _ in high_cardinality_features])
                self.categorical_features = [
                    col for col in self.categorical_features if col not in [c for c, _ in high_cardinality_features]
                ]
        ...
        if len(self.numeric_features) > self.max_features and y is not None:
            self.logger.warning(
                f"Dataset has {len(self.numeric_features):,} numeric features. Pre-selecting top {self.max_features:,} before transformation to prevent memory issues..."
            )
        ...
        if estimated_memory_gb > 4.0:
            ...
            if y is not None and len(self.numeric_features) > 500:
                ...
                self.logger.info(
                    f"Emergency reduction complete: {len(self.numeric_features)} numeric + {len(self.categorical_features)} categorical features"
                )
        ...
        if X_transformed.shape[1] > self.max_features and y is not None:
            self.logger.warning(
                f"Dataset has {X_transformed.shape[1]:,} features. Selecting top {self.max_features:,} features to prevent memory issues..."
            )
            ...
            self.logger.info(
                f"Feature selection complete. Reduced from {len(selected_indices):,} to {len(self.feature_names):,} features"
            )
    ...
    def get_params(self, deep: bool = True) -> Dict[str, Any]:
        ...
        """
        params: Dict[str, Any] = {"max_features": self.max_features, "dimred_config": self.dimred_config}
        return params
