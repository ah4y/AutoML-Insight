...
class ReportBuilder:
    ...
    def generate_report(self, data, profile, results, task_type, recommendation=None) -> str:
        """
        ...
        doc = SimpleDocTemplate(
            str(report_path), pagesize=letter, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=18
        )
        ...
        # Executive summary
        story.extend(self._create_executive_summary(data, profile, results, task_type, recommendation))
        story.append(PageBreak())
    ...
    def _create_title_page(self):
        ...
        subtitle = Paragraph("Automated Machine Learning Analysis", self.styles["Normal"])
        elements.append(subtitle)
        elements.append(Spacer(1, 0.5 * inch))

        date = Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", self.styles["Normal"])
