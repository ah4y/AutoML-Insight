...
class AutoMLDashboard:
    ...
    def render_sidebar(self):
        ...
        if demo_mode:
            ...
            uploaded_file = st.sidebar.file_uploader(
                "Choose a CSV file", type=["csv"], help="Upload your dataset in CSV format", key="dataset_uploader"
            )
        ...
        if st.session_state.data is not None:
            ...
            if task_type == "Classification":
                ...
                if demo_mode and "target" in columns:
                    ...
                    target_col = st.sidebar.selectbox("Select Target Variable", columns, index=len(columns) - 1)
                    st.session_state.target_col = target_col
            ...
            if st.session_state.data is not None:
                ...
                mode, reason, config = CloudExecutor.recommend_execution_mode(
                    n_samples, n_features - (1 if task_type == "Classification" else 0), resources["ram_available_gb"]
                )
    ...
    def load_demo_data(self):
        ...
        demo_choice = st.sidebar.radio("Select Demo Dataset", ["Iris", "Wine"])
    ...
    def render_jupyter_connection(self):
        ...
        with col1:
            if st.button(
                "🔗 Connect", width="stretch", key="jupyter_connect_btn", disabled=st.session_state.jupyter_connected
            ):
        ...
        with col2:
            if st.button(
                "❌ Disconnect",
                width="stretch",
                disabled=not st.session_state.jupyter_connected,
                key="jupyter_disconnect_btn",
            ):
    ...
    def generate_cloud_notebook(self):
        ...
        try:
            ...
            notebook_content = CloudExecutor.generate_colab_notebook(
                dataset_name=dataset_name, target_column=target_col, task_type=task_type, max_features=max_features
            )
            ...
            uploaded_results = st.file_uploader("Choose results file", type=["json"], key="cloud_results_upload")
...
""")
    ...
    def render_configuration_dashboard(self):
        ...
        config_tab1, config_tab2, config_tab3, config_tab4, config_tab5 = st.tabs(
            ["📊 Dataset Analysis", "🤖 Model Selection", "⚙️ Optimization", "🔧 Advanced", "🚀 Execute"]
        )
...
Ensemble: {auto_config['optimization']['include_ensemble']}
    ...
    def _render_advanced_tab(self):
        ...
        with col2:
            ...
            remove_low_variance = st.checkbox(
                "Remove Low Variance Features", value=True, help="Remove features with very low variance"
            )
        ...
        if dimred_enable != "off":
            ...
            with col1:
                dimred_method = st.selectbox(
                    "Method", options=["auto", "pca", "tsvd", "ipca"], index=0, help="Dimensionality reduction method"
                )
            ...
            with col3:
                k_max = st.number_input(
                    "Max Components", min_value=10, max_value=1000, value=256, help="Maximum number of components"
                )
        ...
        with col1:
            cv_folds = st.slider(
                "CV Folds", min_value=3, max_value=20, value=5, help="Number of cross-validation folds"
            )

        with col2:
            test_size = st.slider(
                "Test Size", min_value=0.1, max_value=0.4, value=0.2, step=0.05, help="Proportion of data for testing"
            )
        ...
        with col1:
            n_jobs = st.slider("Parallel Jobs", min_value=-1, max_value=16, value=1, help="-1 uses all available cores")

            enable_caching = st.checkbox(
                "Enable Caching", value=True, help="Cache intermediate results for faster re-runs"
            )

        with col2:
            memory_limit_gb = st.slider(
                "Memory Limit (GB)", min_value=1, max_value=32, value=8, help="Maximum memory usage limit"
            )

            gpu_enabled = st.checkbox(
                "Enable GPU (if available)", value=False, help="Use GPU acceleration for compatible models"
            )
    ...
    def _render_execution_tab(self):
        ...
        if st.session_state.get("dataset_config"):
            ...
            if training_time["category"] == "slow":
                st.warning(
                    "⚠️ This configuration may take significant time. Consider reducing optimization time or number of models."
                )
            elif training_time["category"] == "fast":
