...
class TestEndToEndClassificationPipeline:
    ...
    def test_pipeline_with_mixed_data_types(self):
        ...
        # Combine
        X = pd.concat([
            pd.DataFrame(numerical, columns=[f'num_{i}' for i in range(20)]),
            categorical
        ], axis=1)
...
class TestPerformanceAndScaling:
    ...
    def test_large_dataset_handling(self):
        ...
        config_dict = {
            'dimred': {
                'enable': 'auto',
                'method': 'auto',  # Should select appropriate method
                'k_max': 50
            }
        }
        dimred_config = DimRedConfig(config_dict)
