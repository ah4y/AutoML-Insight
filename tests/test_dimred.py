...
from core.dimred import DimRedConfig, DimRedSelector, make_dimred, select_dimred_method, should_enable_dimred
...
class TestDimRedConfig:
    ...
    def test_invalid_enable(self):
        ...
        config_dict = {"dimred": {"enable": "invalid"}}
    ...
    def test_invalid_method(self):
        ...
        config_dict = {"dimred": {"method": "invalid"}}
    ...
    def test_invalid_variance_target(self):
        ...
        config_dict = {"dimred": {"variance_target": 1.5}}  # Too high
    ...
    def test_invalid_k_max(self):
        ...
        config_dict = {"dimred": {"k_max": 1}}  # Too low
...
class TestShouldEnableDimred:
    ...
    def test_auto_mode_high_dimensional(self):
        ...
        # High feature-to-sample ratio (>0.5)
        assert should_enable_dimred(51, 100, False, "auto") is True
...
class TestSelectDimredMethod:
    ...
    def test_large_dense_data_ipca(self):
        ...
        method = select_dimred_method(False, 50001, "auto")  # >50000 triggers ipca
        assert method == "ipca"
...
class TestMakeDimred:
    ...
    def test_pca_creation(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "k_max": 10, "whiten": True, "seed": 42}}
        config = DimRedConfig(config_dict)

        transformer = make_dimred(is_sparse_after_ohe=False, n_features=20, n_samples=100, cfg=config)
    ...
    def test_tsvd_creation(self):
        ...
        cfg = DimRedConfig(enable="on", method="tsvd", seed=42)
        transformer = make_dimred(is_sparse_after_ohe=True, n_features=50, n_samples=100, cfg=cfg)
    ...
    def test_ipca_creation(self):
        ...
        cfg = DimRedConfig(enable="on", method="ipca")
        transformer = make_dimred(is_sparse_after_ohe=False, n_features=50, n_samples=100, cfg=cfg)
    ...
    def test_invalid_method(self):
        ...
        with pytest.raises(ValueError, match="Invalid method value"):
            DimRedConfig(enable="on", method="invalid")

    def test_pca_with_variance_target(self):
        """Test PCA creation with variance_target."""
        cfg = DimRedConfig(enable="on", method="pca", variance_target=0.95)
        transformer = make_dimred(is_sparse_after_ohe=False, n_features=50, n_samples=100, cfg=cfg)
    ...
    def test_auto_method_selection(self):
        """Test auto method selection for dense vs sparse data."""
        # Dense data -> should get PCA
        cfg = DimRedConfig(enable="on", method="auto")
        transformer = make_dimred(is_sparse_after_ohe=False, n_features=50, n_samples=100, cfg=cfg)
        assert isinstance(transformer, PCA)

        # Sparse data -> should get TruncatedSVD
        transformer_sparse = make_dimred(is_sparse_after_ohe=True, n_features=50, n_samples=100, cfg=cfg)
...
class TestDimRedSelector:
    ...
    def test_fit_enabled(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "variance_target": 0.95}}
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
    ...
    def test_transform_enabled(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "k_max": 10}}  # Limit components
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
        ...
        assert X_transformed.shape[1] < X.shape[1]  # Reduced dimensions
    ...
    def test_fit_transform(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "k_max": 5}}
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
        ...
        assert selector.is_fitted is True
        assert X_transformed.shape[1] < 20  # Reduced from 20 features
    ...
    def test_sparse_data_handling(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "auto"}}
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
    ...
    def test_pipeline_integration(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "k_max": 10}}
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
        ...
        pipeline = Pipeline([("dimred", selector), ("classifier", LogisticRegression(random_state=42))])
...
class TestIntegrationScenarios:
    ...
    def test_high_dimensional_dense_data(self):
        ...
        X, y = make_classification(n_samples=200, n_features=500, n_informative=50, n_redundant=50, random_state=42)
    ...
    def test_forced_pca_on_appropriate_data(self):
        ...
        config_dict = {"dimred": {"enable": "on", "method": "pca", "k_max": 20, "whiten": True}}
        config = DimRedConfig(config_dict)
        selector = DimRedSelector(config)
